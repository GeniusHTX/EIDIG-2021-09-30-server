...
from wss import pre_census_income_wss
from wss.EIDIG_wss import DisInstanceResult
...
def remove_informal_dis_ins(dataset_name, num_attribs, benchmark, round_now):
    ...
    protected_attribs = pre_census_income_wss.protected_attribs

    ids_C_a_EIDIG_5_pkl = '../logging_data_wss/generated_dis_instances_back_up/C-a_ids_EIDIG_5_1.pkl'
    ids_C_g_EIDIG_5_pkl = '../logging_data_wss/generated_dis_instances_back_up/C-g_ids_EIDIG_5_1.pkl'
    ids_C_r_EIDIG_5_pkl = '../logging_data_wss/generated_dis_instances_back_up/C-r_ids_EIDIG_5_1.pkl'
    ...
    R = pickle.load(file)
    X = pre_census_income_wss.get_all_data()
    N_R = []
    for r in R:
        seed = r.seed[0]
    ...
    result_directory = '../logging_data_wss/generated_dis_instances_back_up/'
...
def patch_remove_informal_dis_insdataset_name(dataset_name):
...
def retraining_without_majority_voting(dataset_name, approach_name, percent):
    ...
    for benchmark, protected_attribs in [('C-a', [0]), ('C-r', [6]), ('C-g', [7])]:
        print('\n', benchmark, ':\n')
        ids_benchmark_EIDIG_5_pkl = '../logging_data_wss/generated_discriminatory_instances/' + \
    ...
    model_path = '../models_wss/retrained_models/' + model_name
...
def retraining(dataset_name, approach_name, ids):
    ...
    history = model.fit(X_train, y_train, epochs=100, validation_data=(X_val, y_val),
                        callbacks=[keras.callbacks.EarlyStopping(patience=10)])
    model.evaluate(X_test, y_test)
    model.save('../models_wss/retrained_models/' + dataset_name + '_' + approach_name + '_retrained_model.h5')
...
def retraining_modify(dataset_name, approach_name, ids):
    ...
    history = model.fit(X_train, y_train, epochs=100,
                        ...
                        callbacks=[keras.callbacks.EarlyStopping(patience=10)])
    model.evaluate(X_test, y_test)
    model.save('../models_wss/retrained_models/' + dataset_name + '_' +
...
def retraining_with_formal_dis_ins():
    ...
    for benchmark, protected_attribs in [('C-a', [0]), ('C-r', [6]), ('C-g', [7])]:
        print('\n', benchmark, ':\n')
        ids_benchmark_EIDIG_5_formal_pkl = '../logging_data_wss/generated_discriminatory_instances/' + \
...
def main(argv=None):
    # patch_remove_informal_dis_ins()
    # retraining_with_formal_dis_ins()
    dataset_name = 'adult'
    approach_name = 'EIDIG_5'
    ...
    # reproduction
    # ids_C_a_EIDIG_5 = np.load('../logging_data/generated_discriminatory_instances/C-a_ids_EIDIG_5.npy')
    # ids_C_r_EIDIG_5 = np.load('../logging_data/generated_discriminatory_instances/C-r_ids_EIDIG_5.npy')
    # ids_C_g_EIDIG_5 = np.load('../logging_data/generated_discriminatory_instances/C-g_ids_EIDIG_5.npy')
